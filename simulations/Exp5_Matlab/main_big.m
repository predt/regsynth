% MODIFIED BY JEREMY ON 25 JULY 18
...
parfor t =1:T
    ...
    bmat=C\kron(ones(rDT,1),[x1;ones(1,n1)]);
    b_ordered=reshape(bmat(:),k+1,[]);

    sign_sum = reshape(sum(sign(b_ordered)),rDT,[]);
    [tri,c] = find(sign_sum==k+1);
    triangle = zeros(n1,1); triangle(c) = tri;  % a zero for units outside convex hull
    Wnz = zeros(n1,k+1); Wnz(c,:) = b_ordered(:,sign_sum==k+1)'; % non-zero weights
