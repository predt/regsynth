% MODIFIED BY JEREMY ON 18 JULY 18
% Formula for r real
...
parfor t =1:T
    ...
    % 2.2 computes the corresponding bias
    minbiasm = inf;
    Wmopt = zeros(n0,n1);
    optm = 0;
    dim = size(Wmopt); 
...
sqrt(mean([MSEp MSEnp MSEm MSEmopt MSEp_bc MSEnp_bc MSEm_bc MSEmopt_bc]))

sprintf('RMSE average effects')

sqrt(mean([Estp Estnp Estm Estmopt Estp_bc Estnp_bc Estm_bc Estmopt_bc].^2))

sprintf('|Bias|')

abs(mean([Estp Estnp Estm Estmopt Estp_bc Estnp_bc Estm_bc Estmopt_bc]))
