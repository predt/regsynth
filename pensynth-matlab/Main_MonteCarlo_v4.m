% MODIFIED BY JEREMY ON 22 MARCH 2019
...
% weighted regression for the bias-correction
...
parfor t = 1:T
    ...
    for r = rset
        ...
        LambdaRidge = 0;
        % A. Penalized
        Sigmap = diag(sum(Wp(:,:,i),2));
        mu0 = (f0*Sigmap*f0' + LambdaRidge*eye(k+1))\(f0*Sigmap*y0(:,i));
        mu_hat0 = f0'*mu0;
        mu_hat1 = f1'*mu0;
        ...
        % B. Non-Penalized
        Sigmanp = diag(sum(Wnp,2));
        mu0 = (f0*Sigmanp*f0' + LambdaRidge*eye(k+1))\(f0*Sigmanp*y0(:,i));
        mu_hat0 = f0'*mu0;
        mu_hat1 = f1'*mu0;
        ...
        % C. Fixed Matching
        Sigmam = diag(sum(Wm,2));
        mu0 = (f0*Sigmam*f0' + LambdaRidge*eye(k+1))\(f0*Sigmam*y0(:,i));
        mu_hat0 = f0'*mu0;
        mu_hat1 = f1'*mu0;
        ...
        Sigmamopt = diag(sum(Wmopt(:,:,i),2));
        mu0 = (f0*Sigmamopt*f0' + LambdaRidge*eye(k+1))\(f0*Sigmamopt*y0(:,i));
        mu_hat0 = f0'*mu0;
        mu_hat1 = f1'*mu0;
        ...
        % E. Pure Synthetic Control
        Sigmapure = diag(sum(Wpure,2));
        mu0 = (f0*Sigmapure*f0' + LambdaRidge*eye(k+1))\(f0*Sigmapure*y0(:,i));
        mu_hat0 = f0'*mu0;
        mu_hat1 = f1'*mu0;
