% MODIFIED BY JEREMY ON 11 SEPTEMBER 2018
...
% commented pure synthetic control, so the code works.
% added x0=sqrt(x0).
...
n1 = 10; % treated
n0 = 100; % control
k = 10; % dimension of covariates
a = 0.10; b = 0.90; h = 0.10; % support parameters
r = 4; % curvature of regression function
...
dthr = 0.0001; % threshold for a nul weight
margin = 25;
...
parfor t =1:T
    ...
    % 4. "Pure" synthetic control (lambda close to zero case)
    %DT = delaunayn(x0'); 
    %[tri,WP] = tsearchn(x0',DT,x1'); % return triangle and weight for each treated. NaN if outside conv. hull
...
Sparsity = mean([Densp Densnp Denspure zeros(T,1) Denmopt zeros(T,1) zeros(T,1) zeros(T,1) zeros(T,1) zeros(T,1)])';
Results = table(num2str(RMSEindiv,'%.4f'),num2str(RMSEatt,'%.4f'),num2str(Bias,'%.4f'),num2str(Sparsity,'%.4f'),'RowNames',Name);
Results.Properties.VariableNames = {'RMSEindiv' 'RMSEatt' 'Bias' 'Sparsity'}
